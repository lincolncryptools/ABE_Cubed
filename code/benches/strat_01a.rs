...
fn get_params() -> BenchParams {
    BenchParams::large_strat_01a()
}

fn get_input_generators(_params: &BenchParams) -> Vec<InputGenerator> {
    let mut gens = Vec::new();
    let sizes = (1..=100).collect::<Vec<usize>>();
    for degree in vec![0, 1, 5, 10] {
        gens.push(
            InputGenerator::vary_size(sizes.clone(), degree)
        );
    }
    gens
}
